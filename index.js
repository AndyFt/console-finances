...
// The net total amount of Profit / Losses over the entire period.
  // Need a variable for Profits/Losses (netProfit)
...
// inicialize the greatest variables before the loop
var greatestIncrease = {amount: 0, date:''};
var greatestDecrease = {amount: 0, date:''};

// it needs to start from the second, so i=1
// it will count all the months, so i<totalMonths
// it will increase one until the end, so i++
// the netProfitLoss will keep being reassigned until it sums up everything

var netProfitLoss = finances[0][1];
var previous = netProfitLoss;

for (let i = 1; i < finances.length; i++) {
  netProfitLoss = netProfitLoss + finances[i][1];
  change = finances[i][1] - previous;

  // I need to update the Greatests variables
...
avg = totalChange / (finances.length - 1);
